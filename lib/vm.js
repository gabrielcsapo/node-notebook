...
var _self = module.exports = {
    ...
    run: function(script, callback, error, type) {
        try {
            _self._run(script, function(result) {
                callback(result);
            }, type);
        } catch (ex) {
            if (error && (ex.toString() === error.toString())) {
                ...
                if (ex && ex.code && ex.code === 'MODULE_NOT_FOUND') {
                    try {
                        ...
                        // TODO: this is slow, just call the npm executable
                        npm.load({
                            loaded: false
                        }, function() {
                            npm.commands.install([module], function(error) {
                                ...
                                _self.run(script, callback, ex, type);
                            });
                        });
                    } catch (ex) {
                        return callback({
                    ...
                    _self.run(script, function(result) {
                        callback(result);
                    }, ex, 'async');
                } else {
                    return callback({
    ...
    _run: function(_code, callback, type) {
        var vms = {};
        vms.trace = [];
        vms.vm = vm.createContext({
            ...
            // TODO: this is probably not safe either
            __dirname: __dirname,
